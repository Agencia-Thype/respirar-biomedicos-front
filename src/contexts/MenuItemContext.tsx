...
import { IMenuItemContext, IMenuItemInterfaceData, IMenuItemMutation, IMenuItemUpdateMutation } from "../interfaces/menuItem.interfaces";
...
export const MenuItemContext = createContext<IMenuItemContext>({} as IMenuItemContext);

export const MenuItemProvider = ({ children }: IProvider) => {
  const { refetch: refetchCategories } = useContext(CategoriesContext);
  const [ menuItemDeatilData, setMenuItemDetailData] = useState<IMenuItemInterfaceData | undefined>();
  ...
  const { mutate: updateMenuItem } = useMutation(
    async ({ newData, itemId }: IMenuItemUpdateMutation): Promise<IMenuItemInterfaceData> => {
      const token = localStorage.getItem("@DownTown:Token");
      api.defaults.headers.common["Authorization"] = `Bearer ${token}`;
